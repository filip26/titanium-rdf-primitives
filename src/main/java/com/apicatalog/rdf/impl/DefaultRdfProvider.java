...
public final class DefaultRdfProvider extends RdfProvider {
    ...
    private static final Collection<String> CAN_READWRITE = Arrays.asList(RdfConstants.N_QUADS_MEDIA_TYPE);
    ...
    @Override
    public RdfReader createReader(final String contentType, final Reader reader) throws UnsupportedContentException {

        if (reader == null || contentType == null) {
            throw new IllegalArgumentException();
        }

        if(RdfConstants.N_QUADS_MEDIA_TYPE.equals(contentType)) {
    ...
    @Override
    public RdfWriter createWriter(final String contentType, final Writer writer) throws UnsupportedContentException {
    ...
    @Override
    public Collection<String> canRead() {
        return CAN_READWRITE;
    }

    @Override
    public Collection<String> canWrite() {
        return CAN_READWRITE;
    }

    private static final boolean isBlank(String value) {
        return value.trim().isEmpty() 
                && value.trim().chars().noneMatch(ch ->  ch == ' ' || ch == '\n' || ch == '\r' || ch == '\t' || ch == '\f');
    }
}